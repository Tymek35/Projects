 ...
 * Plik naglowkowy dostarczony przez autora zadania.
...
/**
 * Nadanie nazwy strukturze PhoneNumbers.
 */
typedef struct PhoneNumbers pnT;

/**
 * Nadanie nazwy strukturze PhoneForward.
 */
typedef struct PhoneForward pfT;
...
/** @brief Oblicza ilość nietrywialnych numerów długości len jakie można stworzyć z podanego zbioru.
 * Nietrywialny numer to taki, dla którego funkcja phfwdReverse zwraca coś więcej niż jego
 * samego.
 * @param[in] pf - wskaźnik na strukturę przechowującą przekierowania numerów;
 * @param[in] set - wskażnik na zbiór znaków do tworzenia numerów;
 * @param[in] len - długość numeru tworzonego z cyfr ze zbioru set.
 * @return Liczba nietrywialnych numerów długości len.
 */
